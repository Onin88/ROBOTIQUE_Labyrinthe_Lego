...
def colorSensorCheckFirst(colorSensor):
    not_calibrated = True
    while (not_calibrated):
        try:
            colorSensor.calibrate_white()
            # not good if one of the max is zero
            not_calibrated = ( (colorSensor.red_max == 0) 
                                or (colorSensor.green_max == 0)
                                or (colorSensor.blue_max == 0) )
        except:
            not_calibrated = True
...
def colorIsRed(colorSensor):
    if(colorSensor.color_name == 'Red'):
        return True
    else:
        return False
    
def colorIsBlack(colorSensor):
    if(colorSensor.color_name == 'Black'):
        return True
    else:
        return False
    
def colorIsGreen(colorSensor):
    if(colorSensor.color_name == 'Green'):
        return True
    else:
        return False
    
def colorIsBrown(colorSensor):
    if(colorSensor.color_name == 'Brown'):
        return True
    else:
        return False
    
def colorIsWhite(colorSensor):
    if(colorSensor.color_name == 'White'):
        return True
    else:
        return False
    
def colorIsBlue(colorSensor):
...
def main(noisy = True):
    ...
    color_sensor2 = ColorSensor('in2')
    colorSensorCheckFirst(color_sensor)
    colorSensorCheckFirst(color_sensor2)
    ...
    while ( redExit == False ):
        #if red exit found
        if(colorIsRed(color_sensor) or colorIsBrown(color_sensor) ):
            #stop motors and speed
            steer_motors.off()
            redExit = True
            sound.speak('I found the exit red color') #say that he found the exit
            
        #if the robot follows the black line
        elif((colorIsBlack(color_sensor)) and (anotherWay == False)):
            #acceleration
            speed = 20
            steer_motors.on(steer, speed)
            anotherWay = colorIsBlack(color_sensor2)

        else: #else if the robot is not following black line or on red exit
            speed = 0
            steer_motors.on(steer, speed)
            ...
            while(find == False):
                ...
                time.sleep(0.5)
                find = colorIsBlack(color_sensor)
            find = False
            anotherWay = False
